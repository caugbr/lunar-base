...
<div class="admin-card">
    ...
    <div class="shortcodes-explanation">
        ...
        <div class="shortcodes-usage-grid">
            ...
            <div class="usage-card">
                <h5><x-lucide-code-2 class="lucid-icon" /> Tag com fechamento</h5>
                ...
                <tr data-type="{{ strtolower($data['type']) }}">
                    ...
                    <td class="shortcode-description">
                        ...
                        <div class="shortcode-attributes-list">
                            ...
                            <ul>
                                @foreach($data['attributes'] as $attrName => $attr)
                                <li>
                                    <code>{{ $attrName }}</code>
                                    @if(!empty($attr['label'])) — {{ $attr['label'] }} @endif
                                    @if(!empty($attr['type'])) (<em>{{ $attr['type'] }}</em>) @endif
                                    @if(isset($attr['default']) && $attr['default'] !== '')
                                        — Padrão: <code>{{ $attr['default'] }}</code>
                                    @endif
...
<style>
    ...
    .shortcode-attributes-list strong {
        display: block;
        margin-bottom: 0.35rem;
        color: var(--color-text-dark, #1f2937);
        font-size: 0.85rem;
    }
    .shortcode-attributes-list ul {
        margin: 0;
        padding-left: 1.15rem;
        color: var(--color-text-muted, #4b5563);
    }
    .shortcode-attributes-list li {
        margin-bottom: 0.25rem;
    }
    .shortcode-attributes-list li:last-child {
        margin-bottom: 0;
    }
    .shortcode-attributes-list code {
        font-size: 0.8rem;
        background: #f1f5f9;
