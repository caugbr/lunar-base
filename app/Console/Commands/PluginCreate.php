...
class PluginCreate extends Command
{
    ...
    public function handle(): int
    {
        ...
        foreach ($categoriesConfig as $slug => $category) {
            ...
            $choices[$cnt]     = $display; // Começa a contagem do 1
            $slugMap[$display] = $slug;    // Mapeia o texto exato que o choice() vai retornar
            $cnt++;
        }

        // 2. Seleção clássica por números (iniciando em 1)
        $selectedLabels = [];
        if (!empty($choices)) {
            $selectedLabels = $this->choice(
                'Selecione as categorias/tags (digite os números separados por vírgula, ex: 1,3):',
                $choices,
                null,  // default
                null,  // attempts
                true   // multiple
        ...
        // 3. Converte os textos selecionados de volta para os slugs ('editor', 'marketing', etc.)
        $selectedTags = [];
        ...
        // Geração de arquivos com as tags salvas
        $this->generateManifest($pluginPath, $inputName, $description, $selectedTags);
    ...
    protected function generateManifest(string $path, string $inputName, string $description, array $tags = []): void
    {
        $content = json_encode([
